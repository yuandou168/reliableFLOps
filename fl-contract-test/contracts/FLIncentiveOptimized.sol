...
contract FLIncentiveOptimized {
    ...
    constructor(
        ...
        require(_minParticipants > 0, "Min participants must > 0");
    ...
    function _updatePhase() internal {
        if (currentPhase == ContractPhase.CLOSED) return;
        ...
        if (participantList.length >= minParticipants) {
            ...
            autoClosed = true;
            emit PhaseChanged(ContractPhase.CLOSED);
            return;
        }
    ...
    // 新增视图函数
    function participantsNeeded() public view returns (uint256) {
        if (participantList.length >= minParticipants) return 0;
        return minParticipants - participantList.length;
    }
}